...
function buscarAdmins(){
  fetch('https://tccsolutionsapi.webtech-entrega-test.com.br/admin', {
  method: 'GET',
})
  .then((response) => {
...
if(ENV != 'REPLIT'){
  buscarAdmins();
    ...
    // Obtenha a referência para o elemento do corpo da tabela
    let tbody = document.querySelector('.table-vcenter tbody');
    lista_administradores.forEach(item => {
      let row = document.createElement('tr');
    ...
    // Atualiza o total de registros
    document.getElementById('total').textContent = lista_administradores.length;
    document.getElementById('total_users').textContent = lista_administradores.length;

    let paginas = document.getElementById('paginas');
    let totalPaginas = Math.ceil(lista_administradores.length / 10);
    for (let i = 1; i <= totalPaginas; i++) {
      let li = document.createElement('li');
      li.classList.add('page-item');
      let a = document.createElement('a');
